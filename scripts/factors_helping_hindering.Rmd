...
Out of 307 total districts, 284 respondents answered all 3 questions. A breakdown of the totals is below.
...
I accidentally combined 6.1-6.3 into one table that displays all 3 types of factors. The column labeled `Q` indicates which question the factor in that row corresponds to, with Q8 relating to state policy factors, Q9 relating to local community factors, and Q10 relating to external factors. The box at the top of that column allows you to filter the table by the factor you're interested in.
...
Note: A lower mean score means the factor was considered *more* helpful, as "Definitely helps" is coded as 1.
...
The table and figure below displays the same information as 6.4, with a mean score across all categories, an an option to arrange in descending order. It has an additional "Rank" column that shows which factors were deemed most helpful.
...
q10_mean <- mean_score %>% 
  ...
  mutate(Ranking = row_number())
datatable(q10_mean,
          caption = "Table 6.5d State Policy Factors")
```
...
The table below (6.6a) displays the number of districts that chose a given number of factors that help or hinder across all categories (state policy factors, local community factors, and external factors). Out of 23 total factors they could choose from, the maximum selected for both hinders and helps was 22 and the minimum was 1.  
...
count <- clean %>% 
  ...
  mutate(rate = 1) %>% 
  group_by(group, n_selected) %>% 
  summarize(districts = sum(rate), .groups = "drop") %>% 
  mutate(label = paste0("Selected ", n_selected, " factor(s)")) %>% 
  select(-n_selected) %>% 
  pivot_wider(names_from = group,
              values_from = districts) %>% 
  mutate(across(everything(), ~ifelse(is.na(.), 0, .))) 
# display table
datatable(count,
          caption = "Table 6.6a Across all categories",
          colnames = c("", "Helps", "Hinders"),
          rownames = FALSE,
          options = list(searchHighlight = TRUE))
```
...
```{r, include = FALSE, eval = FALSE}
# save table
write.csv(count, "output/tables/table_6.6a.csv", row.names = FALSE)
```
...
count <- clean %>% 
  ...
  filter(!is.na(response)) %>% 
  # count number of districts that chose X number of factors
  group_by(group, n, response) %>% 
  summarize(districts = n_distinct(id), .groups = "drop") %>% 
  mutate(label = case_when(
    response == "Helps" ~ paste0("Selected ", n, " factor(s) that help"),
    response == "Hinders" ~ paste0("Selected ", n, " factor(s) that hinder")
  )) %>% 
  select(-c(n, response)) %>% 
  pivot_wider(names_from = group,
              values_from = districts) %>% 
...
# Display table
datatable(count,
          caption = "Table 6.6b Within each category",
          colnames = c("", "External factors", "Local community factors", "State policy factors"),
          rownames = FALSE,
          filter = 'top')
```
