...
datatable(table_2.1, caption = "Table 2.1")
```
...
datatable(table_2.2b, caption = "Table 2.2b", rownames = FALSE)
```
...
state <- dat %>% 
  ...
  pivot_wider(names_from = state,
              values_from = c(n, pct)) %>% 
  left_join(labels, by = "vars") %>% 
  select(`Goal selected` = label, n_CT, pct_CT, n_MA, pct_MA, n_NH, pct_NH, n_RI, pct_RI, n_VT, pct_VT) 
#build display table
kable(state, 
      caption = "Table 2.3",
      col.names = c("Goal selected", rep(c("N", "%"), 5))) %>% 
  kable_styling(bootstrap_options = "striped",
                    full_width = F,
                    position = "left") %>% 
  add_header_above(
    header = c(" " = 1, "Connecticut" = 2, "Massachusetts" = 2, "New Hampshire" = 2, "Rhode Island" = 2, "Vermont" = 2),
    align = "c"
  )
```
...
This method is different than conditional co-occurrence where the percentage is calculated among the number selecting the first goal (i.e., the percentage of those who selected Goal 1 that then selected Goal 2).  
...
rm(co_counts_df, co_matrix, co_pct_df, mat, pct_matrix)
```
...
table_2.4c <- clean %>% 
  ...
  rename(`All combined goals` = collapsed_goals,
         N = n,
         `%` = pct)
datatable(table_2.4c, caption = "Table 2.4c Full co-occurence table")
```

```{r eval = FALSE, include = FALSE}
#Save static table
write.csv(table_2.4c, "output/tables/table_2.4c.csv", row.names = FALSE)
```